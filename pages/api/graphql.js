...
const resolvers = {
    ...
    Query: {
        person: async (_, args) => {
            const response = await axios.get(`https://swapi.dev/api/people/${args.id}`);
            const person = response.data;
            return person;
        },
        movie: (_, args) => {
            const { id } = args;
            return movies.find(movie => movie.id === parseInt(id)) // Movie.find(id)
        },
        movies: () => {
            return movies;
        }
    ...
    Mutation: {
        addMovie: (_, args) => {
            ...
            movies = [...movies, movie]; // Movie.save()
            return movie;
        }
    }
}
